...
description: Generates OpenAPI 3.0 specification (YAML) from existing source code. Use when creating API documentation from undocumented REST API code, generating OpenAPI spec from Spring Boot @RestController, ASP.NET Core [ApiController], NestJS @Controller, Express routes, or Next.js App Router route.ts files. Produces standards-compliant OpenAPI 3.0 YAML including paths, request/response schemas, authentication (Bearer JWT, API Key, OAuth2), and example values. Useful for generating Swagger UI, client SDKs, and API contracts for frontend teams.
---

# API Spec — OpenAPI 3.0 Generation

> コードから OpenAPI 3.0 仕様書を生成する。後付けドキュメント化の定番。
