...
>
> **Claude Code の場合**: `/perf-review-sql` で自動起動します（貼り付け不要）。

---

以下の指示に従って、SQL パフォーマンスレビューを実施してください。N+1・インデックス漏れ・フルスキャンの3大原因を体系的に洗い出し、報告書を生成してください。
...
> 対象DB（Oracle / PostgreSQL / MySQL / SQL Server）と、パフォーマンス問題の症状（スロークエリ / タイムアウト / 接続数逼迫）を教えてください。
