---
name: mcp-setup-monitoring
description: Sets up a monitoring MCP server (Datadog or Sentry) for Claude Code. Guides through API key configuration, generates .mcp.json, and demonstrates log retrieval, metric queries, and alert inspection workflows. Use when you want Claude Code to investigate incidents, analyze error trends, or query observability data without leaving the editor.
---

# MCP Monitoring Setup (Datadog / Sentry)
...
## Phase 0: Choose Your Monitoring Tool
...
3. Application Key は権限スコープを最小限に設定（後述）
...
| やりたいこと | 必要なスコープ |
|---|---|
| Issues / エラー確認 | `event:read`, `project:read` |
| リリース確認 | `releases` |
| パフォーマンス | `event:read` |
| メンバー確認 | `member:read`, `org:read` |
...
| `401 Unauthorized` | APIキーが誤りまたは期限切れ | キーを確認・再生成 |
...
```
本番環境で今日の14:00〜15:00の間に
service:api-server で ERROR レベルのログを検索して、
最も頻出するエラーメッセージをランキングにしてください
```

```
host:web-01 でメモリ使用率が90%を超えたタイミングを
過去24時間分調べて、原因を推測してください
```

```
昨日のデプロイ後にエラーレートが上がっていないか確認してください
デプロイ前後1時間を比較してください
```
...
```
Issue ID: 1234567 の詳細を見て、
再現手順と修正方針を提案してください
```

```
今週リリースしたバージョン v2.3.1 で新たに発生したエラーだけ
抽出して一覧にしてください
```
...
- **Write 権限スコープを不用意に付与しない**: 監視ツールへの書き込み（アラート削除・インシデントクローズ等）は意図せず実行されるリスクがある
- **個人情報が含まれるログをClaudeに渡さない**: ユーザーのメールアドレス・IPアドレス等が含まれるログは事前にマスク処理すること
