---
name: spec-gen-csharp-oracle
description: Generates detailed design specs (詳細仕様書) for C# × Oracle systems — common in Japanese finance and manufacturing. Use when documenting ASP.NET Core × Oracle applications, generating specs from C# code using ODP.NET (Oracle.ManagedDataAccess), EF Core with Oracle provider, or Dapper with Oracle. Covers Oracle 11g/19c, ROWNUM pagination, NUMBER precision with decimal, DATE type quirks, stored procedure calls, and OracleParameter binding. Extends spec-gen-enterprise-ja.
---
