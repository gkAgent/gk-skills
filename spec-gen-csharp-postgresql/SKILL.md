---
name: spec-gen-csharp-postgresql
description: Generates detailed design specs (詳細仕様書) for C# × PostgreSQL systems — modern .NET cloud-native applications. Use when documenting ASP.NET Core × PostgreSQL applications using EF Core with Npgsql provider, generating specs for C# microservices using Dapper × PostgreSQL, or creating 詳細仕様書 for .NET 6/7/8 backends with PostgreSQL. Covers PostgreSQL-specific features (JSONB, arrays, sequences, RETURNING, text search), Npgsql provider, EF Core migrations, and NpgsqlParameter binding. Extends spec-gen-enterprise-ja.
---
